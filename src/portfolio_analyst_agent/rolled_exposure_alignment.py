...
DEPRECATED single-signal alignment. It keys the latest-algo join by `acid` only and
collapses algo perspectives (audit M2). Use `rolled_exposure_alignment_multisignal`.
...
def build_account_exposure_alignment(
    ...
    warnings.warn(
        "Single-signal account alignment is deprecated (collapses algo perspectives; "
        "audit M2). Use rolled_exposure_alignment_multisignal.",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    algo_index = _latest_algo_by_acid(algo_workbooks) if algo_workbooks else {}

    rows: list[AccountExposureAlignmentRow] = []
    for row in account_rows:
        acid = row["acid"]
        vir_row = vir_index.get(acid)
        algo_row = algo_index.get(acid)
        ...
        rows.append(
            AccountExposureAlignmentRow(
                ...
                algo_perspective=algo_row.algo_perspective if algo_row else None,
                algo_sheet_dimension=algo_row.sheet_dimension if algo_row else None,
                ...
                algo_join_status="matched_to_algo" if algo_row else "missing_in_algo",
            )
        )
    return rows
...
def build_fund_exposure_alignment(
    ...
    warnings.warn(
        "Single-signal fund alignment is deprecated (collapses algo perspectives; "
        "audit M2). Use rolled_exposure_alignment_multisignal.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def _latest_algo_by_acid(workbooks: list[str | Path] | None) -> dict[str, AlgoLatestSignal]:
    if not workbooks:
        return {}

    signals = latest_signals_from_results(parse_multiple_algo_workbooks(list(workbooks)))
    indexed: dict[str, AlgoLatestSignal] = {}
    for signal in sorted(signals, key=lambda item: (item.acid, item.snapshot_date)):
        indexed[signal.acid] = signal
    return indexed
